...
in {
  options.local.services.noisetorch = with lib; {
    ...
    device-id = mkOption {
      type = lib.types.str;
      description = "The noisetorch device id of the microphone you want noisetorch to act on. Can be found with `noisetorch -l`";
  ...
  config = lib.mkIf cfg.enable {
    ...
    systemd.user.services.noisetorch = {
      ...
      serviceConfig = {
        Type = "simple";
        RemainAfterExit = "yes";
        ExecStart = "${cfg.package}/bin/noisetorch -i -s ${cfg.device-id} -t 95";
